 ...
 */
static void gpio_set_cfgpin(feldev_handle *dev, int port_num, int pin_num, int val)
{
...
{
	size_t bufsize = dev->soc_info->scratch_addr - dev->soc_info->spl_addr;
	void *buf = malloc(bufsize);
	aw_fel_read(dev, dev->soc_info->spl_addr, buf, bufsize);
	return buf;
}
...
{
	size_t bufsize = dev->soc_info->scratch_addr - dev->soc_info->spl_addr;
	aw_fel_write(dev, buf, dev->soc_info->spl_addr, bufsize);
	free(buf);
}
			  ...
			  progress_cb_t progress)
{
	...
	aw_fel_write(dev, cmdbuf, dev->soc_info->spl_addr, max_chunk_size);
	...
	while (len > 0) {
		...
		if (chunk_size == max_chunk_size - 8)
			aw_fel_write(dev, cmdbuf, dev->soc_info->spl_addr, 6);
		else
			aw_fel_write(dev, cmdbuf, dev->soc_info->spl_addr, chunk_size + 8);
		aw_fel_remotefunc_execute(dev, NULL);
		aw_fel_read(dev, dev->soc_info->spl_addr + 6, buf8, chunk_size);
...
void aw_fel_spiflash_info(feldev_handle *dev)
{
	...
	spi0_init(dev);
	aw_fel_write(dev, buf, dev->soc_info->spl_addr, sizeof(buf));
	prepare_spi_batch_data_transfer(dev, dev->soc_info->spl_addr);
	aw_fel_remotefunc_execute(dev, NULL);
